        ...
        while True:
            ...
            if connected == True:  # Kết nối thành công thì nhận gói tin
                try:
                    ...
                    if (
                        "SEND_FILE" in received_msg
                    ):  # Nếu client khác gửi tên file thì download file về
                        file_name = received_msg[
                            received_msg.find('"') + 1 : received_msg.rfind('"')
                        ]  # Lấy tên file
                        person = received_msg[
                            : received_msg.index(":")
                        ]  # Tên người gửi
                        ...
                        )  # Dowload file từ hàm đã viết ở file my_ftp
...
class UI:
    ...
    def send_message(self):
        ...
        if connected and msg != "":
            item = QListWidgetItem("\t" * 4 + "[Tôi]: " + msg)  # align right
            self.message.lstMessage.addItem(item)
            self.message.lstMessage.scrollToItem(item)  # Scroll to bottom
            self.message.txtMessage.setText(
                ""
            )  # Xóa nội dung ở txtMessage sau khi bấm enter
        try:
            if msg != "":
                send(f"[{NAME}]: {msg}")  # SEND NAME (nhập từ bàn phím), msg
        except Exception as e:
            alert = QMessageBox()  # Khi server tắt đột ngột => Báo lỗi
...
def send(msg):
    ...
    message += b" " * (HEADER - len(message))  # Thêm dấu cách cho đủ {HEADER} byte
    client.send(message)
